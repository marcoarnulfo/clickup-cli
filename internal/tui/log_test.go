...
func newTestModelOnReport() Model {
	...
	// real app would land instead of no-oping on an empty chain.
...
func TestLogBrowseEntryOpensBrowser(t *testing.T) {
	...
	m = u.(Model)
	if m.screen != screenListBrowser || len(m.nav) == 0 || m.nav[len(m.nav)-1] != screenLog {
		t.Fatalf("browse row should open the browser for log; screen=%v nav=%v", m.screen, m.nav)
	}
}
...
func TestLogKeyLabelsPerStep(t *testing.T) {
	...
	cases := []struct {
		...
		{"mode select", logModeSelect, 0, []string{"1", "2", "3", "?", "ctrl+c", "ctrl+p", "esc"}},
		{"timer pick", logTimerPick, 0, []string{"1", "2", "?", "ctrl+c", "ctrl+p", "esc"}},
		{"list pick", logListPick, 0, []string{"?", "ctrl+c", "ctrl+p", "down", "enter", "esc", "j", "k", "up"}},
		{"task pick", logTaskPick, 0, []string{"?", "ctrl+c", "ctrl+p", "down", "enter", "esc", "j", "k", "up"}},
		{"id input", logIDInput, 0, []string{"ctrl+c", "ctrl+p", "enter", "esc"}},
		{"form duration", logForm, 0, []string{"ctrl+c", "ctrl+p", "enter", "esc"}},
		{"form date", logForm, 1, []string{"ctrl+c", "ctrl+p", "enter", "esc"}},
		{"form note", logForm, 2, []string{"ctrl+c", "ctrl+p", "enter", "esc"}},
		{"form billable", logForm, 3, []string{"?", "N", "Y", "ctrl+c", "ctrl+p", "enter", "esc", "n", "y"}},
		{"timer running", logTimerRunning, 0, []string{"?", "ctrl+c", "ctrl+p", "esc", "s"}},
		{"done", logDone, 0, []string{"?", "ctrl+c", "ctrl+p", "enter", "esc", "r"}},
	}
	for _, c := range cases {
